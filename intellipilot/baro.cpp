...
boolean baro::begin(uint8_t mode) {
...
int32_t baro::computeB5(int32_t UT) {
...
uint32_t baro::readRawPressure(void) {
  uint32_t raw;
...
int32_t baro::readPressure(void) {
...
int32_t baro::readSealevelPressure(float altitude_meters) {
  float pressure = readPressure();
  return (int32_t)(pressure / pow(1.0-altitude_meters/44330, 5.255));
}

float baro::readTemperature(void) {
...
float baro::readAltitude(float sealevelPressure) {
  float altitude;
...
uint16_t baro::read16(uint8_t a) {
  uint16_t ret;
...
void baro::write8(uint8_t a, uint8_t d) {
  Wire.beginTransmission(BMP085_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  Wire.write(a); // sends register address to read from
