...
void imu::setup() {
	...
	// initialize device
	Serial.println(F("\t\tInitializing IMU devices..."));
	mpu.initialize();
	pinMode(IMU_INTERRUPT_PIN, INPUT);

	// verify connection
	Serial.print(F("\t\tTesting IMU connections..."));
	...
	if (devStatus == 0) {
		...
		attachInterrupt(IMU_INTERRUPT_PIN, dmpDataReady, RISING);
...
void imu::getGyro(double *data) {
	...
	if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
		...
		mpu.dmpGetGyro(_gyro, fifoBuffer);
