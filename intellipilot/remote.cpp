...
uint8_t remote::getCommands(float *data) {
		...
		ch1 = mMath.floor(ch1 / ROUNDING_BASE) * ROUNDING_BASE;
		ch2 = mMath.floor(ch2 / ROUNDING_BASE) * ROUNDING_BASE;
		ch3 = mMath.floor(ch3 / ROUNDING_BASE) * ROUNDING_BASE;
		ch4 = mMath.floor(ch4 / ROUNDING_BASE) * ROUNDING_BASE;
		...
		if ((ch1 < YAW_SPEED_MIN) || (ch1 > YAW_SPEED_MAX)) ch1 = ch1Last;
		if ((ch2 < PITCH_ANG_MIN) || (ch2 > PITCH_ANG_MAX)) ch2 = ch2Last;
		if ((ch3 < ROLL_ANG_MIN) || (ch3 > ROLL_ANG_MAX)) ch3 = ch3Last;
		if ((ch4 < MOTOR_PULSE_MIN || (ch4 > MOTOR_PULSE_MAX)) ch4 = ch4Last;
