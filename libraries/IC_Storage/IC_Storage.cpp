...
byte IC_Storage::read(uint32_t address) {
        return FLASH_START[address];
}

byte* IC_Storage::readAddress(uint32_t address) {
        return FLASH_START+address;
}

boolean IC_Storage::write(uint32_t address, byte value) {
        uint32_t retCode;
...
boolean IC_Storage::write(uint32_t address, byte *data, uint32_t dataLength) {
        uint32_t retCode;
